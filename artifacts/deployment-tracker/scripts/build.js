...
async function buildWebExport(projectRoot) {
  console.log("\n── Web export ──────────────────────────────────────────────");
  const webOutputDir = path.join(projectRoot, "static-build", "web");

  // Clean previous web build
  ...
  try {
    execSync(
      `pnpm exec expo export --platform web --output-dir "${webOutputDir}"`,
      {
        cwd: projectRoot,
        stdio: "inherit",
        env: { ...process.env },
    ...
    // Non-fatal — web export failure shouldn't block the native (Expo Go) build
...
async function main() {
  ...
  // Kill Metro before starting web export (avoids port conflicts)
  if (metroProcess) {
    metroProcess.kill();
    metroProcess = null;
  }
  ...
  await buildWebExport(projectRoot);
