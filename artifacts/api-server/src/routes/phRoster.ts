...
const CNY_EXCLUDED       = new Set(["Chuiguang", "Matthew", "Azriel"]);
/** Never scheduled on Deepavali. Rule A & B do not apply to Ben for Deepavali. */
const DEEPAVALI_EXCLUDED = new Set(["Ben"]);
/** Always in the last 4 DAY slots of every Hari Raya; bypass all rules. */
const HR_FIXED_DAY       = ["Ben", "Chuiguang", "Matthew", "Azriel"] as const;
...
const ALL_MUSLIM_OFFICERS: readonly string[] = [
  "Asyir","Faizal","Taufik","Syafiq","Juraimi","Salim Y","Afiq","Suffi",
  "Farhan O","Khamsani","Khairul","Effendy","Shafizam","Haziq","Nash","Rahim",
  "Azlan","Rizdwan","Norhisam","Fitri","Amsyar","Faiz","Rabiul",
 ...
 *   12 slots filled from non-Chinese pool (Chuiguang, Matthew, Azriel excluded).
 ...
 *   Last 4 DAY slots: Ben, Chuiguang, Matthew, Azriel (fixed, no Rule A/B).
 *   Sub-catchment labels come from the rotation sequence (6 units as normal).
 *
 * ─ SCENARIO 2c — Deepavali ───────────────────────────────────────────────────
 *   12 slots filled from non-Hindu pool (Ben excluded).
 *   Rule A & B apply normally to everyone else. Equal distribution maintained.
 *
 * ─ OIL (in-lieu copies) ──────────────────────────────────────────────────────
...
function autoAllocate(
  ...
  const allocateHariRaya = (units: string[]): PHRefRow[] => {
    ...
    const pairs: string[][] = [
      ...
      [HR_FIXED_DAY[0], HR_FIXED_DAY[1]],            // unit 4  DAY  Ben, Chuiguang
      [HR_FIXED_DAY[2], HR_FIXED_DAY[3]],            // unit 5  DAY  Matthew, Azriel
    ];
    return makeRows(units, pairs);
  };
  ...
  // Does NOT relax racial exclusions (Ben/CNY/Deepavali rules remain hard).
  {
