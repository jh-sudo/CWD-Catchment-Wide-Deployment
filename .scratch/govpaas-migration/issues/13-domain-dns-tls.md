...
The app currently has **no custom domain** — Replit's own default subdomain is in use today (`location-tracker-pubpmv9.replit.app`, referenced as the API endpoint in `artifacts/wls-android-forwarder/BUILD_INSTRUCTIONS.md`). There's no existing DNS to migrate; this is a fresh decision, not a lift-and-shift.
