...
The app has **no custom domain today** — Replit's own default subdomain is in use (`your-app.replit.app`). On GOV PaaS:
...
- **deployment-tracker**: already uses `process.env.EXPO_PUBLIC_API_URL` throughout, falling back to `https://your-dashboard.replit.app` if unset.
