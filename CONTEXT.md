...
- Units are selected from a fixed 24-unit rotation sequence in strict order — no unit is ever skipped at the unit level for racial/religious reasons. Racial exclusion (CNY: Chuiguang/Matthew/Azriel excluded; Deepavali: Ben excluded) is applied at the **officer level** inside the selected unit — the unit still appears in the roster with its remaining eligible officer(s).
...
- Hari Raya has its own layout: 4 rotation units → 3 PD + 1 DAY slots via the normal filtering, plus two fixed DAY pairs (Ben+Chuiguang, Matthew+Azriel) that bypass all criteria.
